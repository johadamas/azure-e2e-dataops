...
resource "databricks_cluster" "cluster" {
  cluster_name            = "jonathan-project-cluster"
...
resource "databricks_job" "etl_pipeline" {
  ...
  task {
    ...
    run_if   = "ALL_SUCCESS"
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/1.storageMount.py"].path
      source        = "WORKSPACE"
    }
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/2.ingestBronze.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/3.transformSilver.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/4.transformGold.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0
  ...
  task {
    ...
    notebook_task {
      notebook_path = databricks_notebook.notebooks["${path.module}/5.loadDelta.py"].path
      source        = "WORKSPACE"
    }
    existing_cluster_id = databricks_cluster.cluster.id
    timeout_seconds     = 0
